<?php
namespace Evocative\Elements\Items\IconText;
...
class Element {
  ...
  public function vc_map() {
    ...
    $params_icon = [
      ...
      ['type'=>'textfield','heading'=>__('Icône Dashicons','evoe'),'param_name'=>'icon_dash','description'=>__('ex: dashicons-location','evoe'),'dependency'=>['element'=>'icon_lib','value'=>['dash']]],
      ['type'=>'attach_image','heading'=>__('Icône SVG','evoe'),'param_name'=>'icon_svg','dependency'=>['element'=>'icon_lib','value'=>['svg']]],
    ...
    $params_content = [
      ...
      ['type'=>'dropdown','heading'=>__('Balise texte','evoe'),'param_name'=>'text_tag','value'=>['div','p','h1','h2','h3','h4','h5','h6'],'std'=>'p'],
      ['type'=>'textfield','heading'=>__('Séparateur','evoe'),'param_name'=>'sep','std'=>'•'],
      ['type'=>'textfield','heading'=>__('Espace séparateur','evoe'),'param_name'=>'sep_gap','std'=>'8px'],
    ...
    vc_map([
      'name'=>__('EE • Icon Text','evoe'),
      'base'=>self::SLUG,
      'icon'=>'dashicons-format-status',
      'category'=>__('Evocative Elements','evoe'),
      'description'=>__('Icône + texte style pilule','evoe'),
      'params'=>array_merge(
        $params_icon,
        $params_content,
        $this->chip_params(),
  ...
  public function render($atts) {
    ...
    // Icône
    $icon_html = '';
    if ($a['icon_lib']==='fa' && $a['icon_fa']) {
      $icon_html = '<i class="'.esc_attr($a['icon_fa']).'" aria-hidden="true"></i>';
    } elseif ($a['icon_lib']==='dash' && $a['icon_dash']) {
      $icon_html = '<span class="dashicons '.esc_attr($a['icon_dash']).'" aria-hidden="true"></span>';
    } elseif ($a['icon_lib']==='svg' && $a['icon_svg']) {
      $icon_html = '<span class="evoe-svg" aria-hidden="true">'.Helpers::inline_svg(intval($a['icon_svg'])).'</span>';
    } elseif ($a['icon_lib']==='emoji' && $a['icon_emoji']) {
      $icon_html = '<span class="evoe-emoji" role="img" aria-hidden="true">'.esc_html($a['icon_emoji']).'</span>';
    }

    // Texte + séparateur
    $tag = Helpers::is_allowed_tag($a['text_tag']);
    $sep = strlen(trim($a['sep'])) ? '<span class="evoe-sep" aria-hidden="true">'.esc_html($a['sep']).'</span>' : '';
    $text = '<'.$tag.' class="evoe-text">'.wp_kses_post($a['text']).'</'.$tag.'>';
    ...
    $style = $this->chip_style_vars($a) . '--es-sep-gap:'.$a['sep_gap'].';';

    // inner
    $inner = '<span class="evoe-icon">'.$icon_html.'</span>';
    if ($sep) $inner .= $sep;
    $inner .= '<span class="evoe-content">'.$text.'</span>';
